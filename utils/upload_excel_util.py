...
def upload_file_v2(db: Session, project_id: int, analysis_id: int, excel_id: str, send_mail_address: str):
    try:
        ...
        for _, row in donor_df.iterrows():
            ...
            insert_donor_model_list.append(
                cellxgene.DonorMeta(
                    **donor_meta.model_dump(mode="json", exclude={"id"}, exclude_none=True)
                )
            )

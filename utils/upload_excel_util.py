...
def upload_file_v2(
    ...
    try:
        update_except_gene_expression_meta(db=db, project_id=project_id, analysis_id=analysis_id, excel_id=excel_id)
    except Exception as e:
        mail_util.send_mail(
            mail_template="项目信息更新失败，失败原因：{}".format(str(e)),
            subject="项目信息更新失败",
        ...
        try:
            update_gene_expression_meta(db=db, project_id=project_id, analysis_id=analysis_id, excel_id=excel_id)
        except Exception as e:
            last_excel_id = get_last_update_file_id(db=db, project_id=project_id, analysis_id=analysis_id)
            if last_excel_id:
                update_except_gene_expression_meta(db=db, project_id=project_id, analysis_id=analysis_id, excel_id=last_excel_id)
            mail_util.send_mail(
                mail_template="项目信息更新失败，失败原因：{}".format(str(e)),
                subject="项目信息更新失败",
                to_list=send_mail_address,
            ...
            crud.create_project_update_history(
                ...
                insert_project_update_history_model=cellxgene.ProjectUpdateHistory(
                    project_id=project_id, analysis_id=analysis_id, file_id=excel_id, file_name=file_name
                ),
            )
            crud.update_analysis(db=db, filters=[cellxgene.Analysis.id == analysis_id], update_dict={"excel_id": excel_id})
            mail_util.send_mail(
                mail_template="项目信息更新成功",
                subject="项目信息更新成功",
                to_list=send_mail_address,
            )
    finally:
        logging.info("[update file cost] {} s".format(str(time.time() - start_time)))


def update_except_gene_expression_meta(db: Session, project_id: int, analysis_id: int, excel_id: str):
    file_path = config.H5AD_FILE_PATH + "/" + excel_id
    with open(file_path, "rb") as file:
        project_content = file.read()
    project_excel_df = pd.ExcelFile(BytesIO(project_content))
    ...
    pathway_score_df["cell_type_name"] = pathway_score_df[
        "calculated_cell_cluster_alias_id"
    ].apply(lambda xx: xx.split("_")[3])
    update_project_dict = project_df.to_dict("records")[0]
    ...
    crud.update_project_for_transaction(
        ...
        update_dict=update_project_dict,
    )
    crud.update_analysis_for_transaction(
        db=db,
        filters=[cellxgene.Analysis.id == analysis_id],
        update_dict=update_analysis_dict,
    )
    ...
    inserted_donor_id_list = crud.create_donor_meta_for_transaction(
    ...
    biosample_df["donor_id"] = biosample_df["donor_id"].apply(
        lambda xx: int(inserted_donor_id_dict.get(xx, 0))
    )
    for _, row in biosample_df.iterrows():
    ...
    pathway_score_df["species_id"] = pathway_score_df["species"].apply(
        lambda xx: species_dict[xx.strip()]
    )
    pathway_score_df["biosample_id"] = pathway_score_df["biosample_id"].apply(
        lambda xx: inserted_biosample_id_dict.get(xx, 0)
    )
    ...
    for i in range(1, pathway_insert_count + 1):
        crud.create_pathway_score_for_transaction(
            db=db,
            insert_pathway_meta_list=insert_pathway_model_list[
                                     write_count * (i - 1): write_count * i
                                     ],
        )
    crud.create_pathway_score_for_transaction(
        db=db,
        insert_pathway_meta_list=insert_pathway_model_list[
                                 pathway_insert_count * write_count:
                                 ],
    )
    db.commit()


def update_gene_expression_meta(db: Session, project_id: int, analysis_id: int, excel_id: str):
    file_path = config.H5AD_FILE_PATH + "/" + excel_id
    with open(file_path, "rb") as file:
        project_content = file.read()
    project_excel_df = pd.ExcelFile(BytesIO(project_content))
    ...
    gene_expression_df = gene_expression_df.replace(np.inf, 3.91e303)
    gene_expression_df["gene_symbol"] = gene_expression_df["gene_symbol"].astype(
        str
    )
    insert_gene_expression_model_list = []
    for _, row in gene_expression_df.iterrows():
    ...
    gene_expression_insert_count = int(
        len(insert_gene_expression_model_list) / write_count
    )
    ...
    crud.create_gene_expression_for_transaction(
        db=db,
        insert_gene_expression_model_list=insert_gene_expression_model_list[
                                          gene_expression_insert_count * write_count:
                                          ],
    )
    db.commit()


def get_last_update_file_id(db: Session, project_id: int, analysis_id: int):
    last_file_id = ""
    project_update_history_meta = crud.get_project_update_history(
        db=db,
        query_list=[cellxgene.ProjectUpdateHistory],
        filters=[cellxgene.ProjectUpdateHistory.project_id == project_id,
                 cellxgene.ProjectUpdateHistory.analysis_id == analysis_id],
    ).order_by(cellxgene.ProjectUpdateHistory.create_at.desc()).first()
    if project_update_history_meta:
        last_file_id = project_update_history_meta.file_id
    return last_file_id
...
if __name__ == "__main__":
    ...
    #     project_id=28,
    #     analysis_id=28,
    #     excel_id="84bb8e668bfb4323819ca89abc0b0137.xlsx",
    #     send_mail_address="619589351@qq.com",
    # )
    get_last_update_file_id(
        db=next(get_db()),
        project_id=29,
        analysis_id=28,
    )
